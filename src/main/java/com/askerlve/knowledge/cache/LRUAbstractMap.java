package com.askerlve.knowledge.cache;
...
/**
 * @author Askerlve
 * @Description: 1.在做 key 生成 hashcode 时是用的 HashMap 的 hash 函数
...
public class LRUAbstractMap extends AbstractMap {
    ...
    public LRUAbstractMap() {
        ...
        this.arrays = new Object[this.arraySize];
        //开启一个线程检查最先放入队列的值是否过期
        this.executeCheckTime();
    }
    ...
    public Object put(Object key, Object value) {
        ...
        Node currentNode = (Node) this.arrays[index];

        if (Objects.isNull(currentNode)) {
            arrays[index] = new Node(null, null, key, value);
            this.QUEUE.offer((Node) arrays[index]);
            ...
            while (nNode.next != null) {
                ...
                if (nNode.key == key) {
                    ...
                    //不存在就新增链表
                    this.sizeUp();
                    Node node = new Node(null, nNode, key, value);
                    //写入队列
                    this.QUEUE.offer(currentNode);
                    cNode.next = node;
                }
